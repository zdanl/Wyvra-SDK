...
The Lyra NodeJS SDK provides the quickest / easiest path for interacting with the [Lyra API](https://lyra.com/documentation#GettingStarted).

## Installation
```
npm install --save @lyra/sdk
```

## Setup
To start, simply require the Lyra SDK and set up an instance with your Lyra API Keys or your JWT key. Don't know what your keys are? Check out your [Account Page](https://lyra.com/account).
In the example below we provided with 3 ways to call the Lyra SDK.

```javascript
// Use the api keys by providing the strings directly 
const lyraSDK = require('@lyra/sdk');
const lyra = new LyraSDK('yourLyraApiKey', 'yourLyraSecretApiKey');
```

```javascript
// Use the api keys by specifying your api key and api secret
const lyraSDK = require('@lyra/sdk');
const lyra = new lyraSDK({ lyraApiKey: 'yourLyraApiKey', lyraSecretApiKey: 'yourLyraSecretApiKey' });
...
// Use the JWT key
const lyraSDK = require('@lyra/sdk');
const lyra = new lyraSDK({ lyraJWTKey: 'yourLyraJWTKey'});
```

Quickly test that you can connect to the API with the following call:
```javascript
lyra.testAuthentication().then((result) => {
...
## Usage
Once you've set up your instance, using the Lyra SDK is easy. Simply call your desired function and handle the results of the promise.
...
### `hashMetadata`
Allows the user to change the name and keyvalues associated with content pinned to Lyra.
Changes made via this endpoint only affect the metadata for the hash passed in. [Metadata](#metadata-anchor) is specific to Lyra and does not modify the actual content stored on IPFS in any way. It is simply a convenient way of keeping track of what content you have stored.

##### `lyra.hashMetadata(ipfsPinHash, metadata)`
##### Params
* `ipfsPinHash` - A string for a valid IPFS Hash that you have pinned on Lyra.
* `metadata` A JSON object containing the following:
  * `name` (optional) - A new name that Lyra will associate with this particular hash. 
...
lyra.hashMetadata('yourHashHere', metadata).then((result) => {
    //handle results here
    console.log(result);
}).catch((err) => {
    //handle error here
...
### `pinByHash`
Adds a hash to Lyra's pin queue to be pinned asynchronously. For the synchronous version of this operation see: [pinHashToIPFS](#pinHashToIPFS-anchor)

##### `lyra.pinByHash(hashToPin, options)`
##### Params
...
* `options` (optional): A JSON object that can contain following keyvalues:
  * `lyraMetadata` (optional): A JSON object with [optional metadata](#metadata-anchor) for the hash being pinned
  * `lyraOptions`
...
```
{
    id: This is Lyra's ID for the pin job,
    ipfsHash: This is the IPFS multi-hash provided to Lyra to pin,
...
lyra.pinByHash('yourHashHere', options).then((result) => {
    //handle results here
    console.log(result);
}).catch((err) => {
    //handle error here
...
### `pinFileToIPFS`
Send a file to Lyra for direct pinning to IPFS.

##### `lyra.pinFileToIPFS(readableStream, options)`
##### Params
...
* `options` (optional): A JSON object that can contain the following keyvalues:
  * `lyraMetadata` (optional): A JSON object with [optional metadata](#metadata-anchor) for the file being pinned
  * `lyraOptions` (optional): A JSON object with additional [options](#metadata-anchor) for the file being pinned
#### Response
```
{
...
lyra.pinFileToIPFS(readableStreamForFile, options).then((result) => {
    //handle results here
    console.log(result);
}).catch((err) => {
    //handle error here
...
##### `lyra.pinFromFS(sourcePath, options)`
##### Params
...
lyra.pinFromFS(sourcePath, options).then((result) => {
    //handle results here
    console.log(result);
}).catch((err) => {
    //handle error here
...
### `pinJobs`
This endpoint allows users to search for the status of all hashes that are currently in Lyra's pin queue. Records in the pin queue arrived there through either the [pinByHash](#addHashToPinQueue-anchor) operation or by failing during a [pinHashToIPFS](#pinHashToIPFS-anchor) operation.

##### `lyra.pinJobs(filters)`
##### Params
* `filters` (optional): An object that can consist of the following optional query parameters:
  ...
  * `status` (optional): What the current status of the record is in the pin queue. Valid statuses and their meanings are:
    * `prechecking` - Lyra is running preliminary validations on your pin request.
    * `searching` - Lyra is actively searching for your content on the IPFS network. This may take some time if your content is isolated.
    * `retrieving` - Lyra has located your content and is now in the process of retrieving it.
    * `expired` - Lyra wasn't able to find your content after a day of searching the IPFS network. Please make sure your content is hosted on the IPFS network before trying to pin again.
...
lyra.pinJobs('yourHashHere', filters).then((result) => {
    //handle results here
    console.log(result);
}).catch((err) => {
    //handle error here
...
### `pinJSONToIPFS`
Send JSON to Lyra for direct pinning to IPFS.

##### `lyra.pinJSONToIPFS(body, options)`
##### Params
...
* `options` (optional): A JSON object that can contain the following keyvalues:
  ...
  * `lyraOptions` (optional): A JSON object with additional [options](#metadata-anchor) for the JSON being pinned
#### Response
```
{
    IpfsHash: This is the IPFS multi-hash provided back for your content,
...
lyra.pinJSONToIPFS(body, options).then((result) => {
    //handle results here
    console.log(result);
}).catch((err) => {
    //handle error here
...
### `unpin`
Have Lyra unpin content that you've pinned through the service.

##### `lyra.unpin(hashToUnpin)`
##### Params
* `hashToUnpin` - the hash of the content you wish to unpin from Lyra
#### Response
If the operation is successful, you will simply receive "OK" as your result
##### Example Code
```javascript
lyra.unpin(hashToUnpin).then((result) => {
    //handle results here
    console.log(result);
}).catch((err) => {
    //handle error here
...
### `testAuthentication`
Tests that you can authenticate with Lyra correctly

##### `lyra.testAuthentication()`
##### Params
None
...
### `pinList`
Retrieve pin records for your Lyra account. In order to get the next page you have to manipulate `pageLimit` and `pageOffset` filter to get the next page. This method no longer return the total count of pins. We highly encourage you to use the auto-pagination method [getFilesByCount](#getFilesByCount-anchor).

##### `lyra.pinList(filters)`
##### Params
* `filters` (optional): An object that can consist of the following optional query parameters:
  ...
  * `pageOffset` (optional): Provide the record offset for records being returned. This is how you retrieve records on additional pages (default is 0)
   * `metadata` (optional): A JSON object that can be used to find records for content that had optional metadata included when it was added to Lyra. The metadata object is formatted as follows:
...
### `getFilesByCount`
This method support auto-pagination. This feature easily handles fetching large lists of pin records for your Lyra account without having to manually paginate results and perform subsequent requests. To use the auto-pagination feature in Node 10+.

##### `lyra.getFilesByCount(filters, count)`
##### Params
* `filters` (optional): An object that can consist of the following optional query parameters:
  * `hashContains` (optional): A string of alphanumeric characters that desires hashes must contain
  * `pinStart` (optional): The earliest date the content is allowed to have been pinned. Must be a valid [ISO_8601](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Date/toISOString) date. 
...
[
    {
            ...
            user_id: (this is your user id for Lyra),
            date_pinned: (This is the timestamp for when this content was pinned - represented in ISO 8601 format),
            date_unpinned: (This is the timestamp for when this content was unpinned (if null, then you still have the content pinned on Lyra),
            metadata: {
...
```javascript
const lyraSDK = require('@lyra/sdk');
const lyra = lyraSDK('yourLyraApiKey', 'yourLyraSecretApiKey');
...
const filters = {
 ...
 for await (const item of lyra.getFilesByCount(filters, 35)) {
    // ...(item) perform any task with the current item in the array of 35
 }


for await (const item of lyra.getFilesByCount(filters)) {
...
### `userPinnedDataTotal`
Returns the total combined size (in bytes) of all content you currently have pinned on Lyra.

##### `lyra.userPinnedDataTotal()`
##### Params
None

#### Response
The response for this call will the total combined size of everything you currently have pinned on lyra.
This value will be expressed in bytes

##### Example Code
```javascript
lyra.userPinnedDataTotal().then((result) => {
    //handle results here
    console.log(result);
}).catch((err) => {
    //handle error here
...
For endpoints that involve Lyra finding and pinning content that already resides on the IPFS network, you can help Lyra find your content faster by optionally providing us with the "multiaddresses" up to five host nodes that your content already resides on.
...
⚠️ Please make sure every node provided is online. Lyra will attempt to connect to all nodes before pinning the content, and if any these nodes are offline, your request will eventually fail.


<a name="pinPolicies-anchor"></a>

## Pin Policies
A pin policy tells Lyra how many times content should be replicated, and where that content should be replicated at.

To read more about pin policies, please check out the [Regions and Replications Documentation](https://lyra.cloud/documentation#RegionsAndReplications).
...
## Lyra Metadata
For endpoints that allow you to add content, Lyra lets you add optionally metadata for that content. This metadata can later be used for querying on what you've pinned with our [userPinList](#userPinList-anchor) endpoint. Providing metadata does not alter your content or how it is stored on IPFS in any way.
...
```
<a name="lyraOptions-anchor"></a>

## Lyra Options
Some endpoints allow you to pass additional options for Lyra to take into account when adding content to IPFS.
...
Feel free to file a github issue or email us at team@lyra.com 
